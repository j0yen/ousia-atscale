//! Integration tests covering AC #1–#8.

use ousia_atscale::{
    annotate::{emit_overlay, GroundedOverlay},
    mapper::{BfoCategory, Mapper},
    model::{AtscaleModel, ElementKind},
...
fn test_measure_maps_to_information_gdc() {
    let mapper = Mapper::new();
    let elem = ousia_atscale::model::ModelElement {
        name: "revenue",
...
fn test_report_coverage_fallback_counted() {
    ...
    use ousia_atscale::model::ModelElement;
    let mapper = Mapper::new();
    ...
    let grounded = vec![mapper.ground(&elem)];
    let report = CoverageReport::build(&grounded);
